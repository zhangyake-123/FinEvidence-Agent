...
def read_text(path: str | Path) -> str:
    """Read an HTML filing with a forgiving encoding strategy."""

    return Path(path).read_text(encoding="utf-8", errors="ignore")


def html_to_text(html: str) -> str:
    """Convert SEC HTML to readable text."""
...
def make_filing_record(path: str | Path) -> dict:
    """Build one metadata record for a raw filing."""
...
def parse_filing_file(path: str | Path, max_chars: int = 4000, overlap_chars: int = 400) -> tuple[dict, list[dict]]:
    """Parse one HTML filing into metadata and text chunk records."""
...
def parse_filings(
    ...
    """Parse all downloaded filings and write filing index plus text chunks."""
